...
import com.github.wallev.maidsoulkitchen.chest.TomSimpleStorageContainer;
...
public final class MaidPlugin implements ILittleMaid {
    ...
    public void addChestType(ChestManager manager) {
        ...
        if (Mods.TSS.isLoaded()) {
            manager.add(new TomSimpleStorageContainer());
        }
        if (Mods.HANDCRAFTED.isLoaded()) {
            manager.add(new HandCraftedContainer());
        }
    }
