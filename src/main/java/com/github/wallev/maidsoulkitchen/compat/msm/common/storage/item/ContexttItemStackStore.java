...
public class ContexttItemStackStore extends AbstractFilterableBlockStorage implements IStorageInsertableContext, IStorageSplitInsertableContext {
    private SimulateItemStackInteractHelper helper;


    @Override
    public void start(EntityMaid maid, ServerLevel level, Target target) {
        super.start(maid, level,target);
        helper = new SimulateItemStackInteractHelper(maid, target.pos, target.side, level);
        helper.open();
    }
